...
export const ScuolaSection = () => {
  ...
  // Derived lists for Scadenze sorted by closest deadline first (chi scade prima)
  const pendingItems = schoolItems
    .filter(i => i.status !== 'completato')
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  const completedItems = schoolItems.filter(i => i.status === 'completato');
  ...
  return (
    <div className="w-full px-4 md:px-8 pt-5 pb-16 mb-12 space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-neutral-200/80 dark:border-neutral-800/80">
        <div>
          ...
          <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5">
            Registro voti per materia, scadenze attive e attività completate.
          </p>
        </div>
        ...
        {activeTab === 'voti' ? (
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-medium flex items-center gap-1.5 shadow-xs transition-colors"
          >
            <Plus className="w-3.5 h-3.5" />
      ...
      <div className="flex items-center gap-1.5 border-b border-neutral-200 dark:border-neutral-800 pb-1.5">
        <button
      ...
      {activeTab === 'voti' && (
        <div className="space-y-4 animate-fade-in w-full">
          ...
          <GradesChart grades={grades} subjects={INITIAL_SUBJECTS} />
